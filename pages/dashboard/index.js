...
import {
  getPublicLectures, updateLecture, getPublicLectureSeries, updateLectureSerie,
} from '@utils/API';
...
const DashboardReal = () => {
  ...
  const {
    category, time, price, status,
  } = input;

  let filteredLectures = !isEmpty(lectures.data) ? category !== 'courses' ? orderBy(lectures.data, (object) => new Date(object.createdAt), ['desc']) : orderBy(lectureseries.data, (object) => new Date(object.createdAt), ['desc']) : [];

  if (category !== 'courses') {
    if (price) {
      switch (price) {
        case 'free':
          filteredLectures = filter(filteredLectures, (found) => found.price === 0);
          break;
        case '0-1000':
          filteredLectures = filter(filteredLectures, (found) => found.price < 1001 && found.price > 0);
          break;
        case '1000-2000':
          filteredLectures = filter(filteredLectures, (found) => found.price < 2001 && found.price > 1000);
          break;
        case '2000+':
          filteredLectures = filter(filteredLectures, (found) => found.price > 2000);
          break;
        default:
          break;
      }
    ...
    if (status) {
      switch (status) {
        case 'live':
          filteredLectures = filter(filteredLectures, (found) => found.meetingInfo.running[0] === 'true');
          break;
        case 'thisweek':
          filteredLectures = filter(filteredLectures, (found) => new Date(found.startTime).getTime() < new Date().getTime() + 7 * 24 * 60 * 60 * 1000);
          break;
        default:
          break;
      }
    ...
    if (time) {
      switch (time) {
        case 'nearest':
          filteredLectures = orderBy(filteredLectures, (object) => new Date(object.startTime), ['asc']);
          break;
        case 'farthest':
          filteredLectures = orderBy(filteredLectures, (object) => new Date(object.startTime), ['desc']);
          break;
        default:
          break;
      }
    ...
    switch (time) {
      case 'nearest':
        filteredLectures = orderBy(filteredLectures, (object) => new Date(object.createdAt), ['desc']);
        break;
      case 'farthest':
        filteredLectures = orderBy(filteredLectures, (object) => new Date(object.createdAt), ['asc']);
        break;
      default:
        break;
    }
  ...
  if (user.role === 'student') {
    ...
    return <main className={(!user.role || user.role === 'student') ? 'pl-0 pt-5 w-75 mx-auto mainStudent' : 'pt-5 mx-lg-5'}>
      <Head>
        <title>Lectures | SchoolX, the leading online learning platform in Pakistan</title>
      </Head>
      <div className="container-fluid mt-5">
        {notifMessage ? (
          <div className={`alert alert-${notifMessageType} my-3 mx-4`} role="alert">
            {notifMessage}
          </div>
        ) : null}
        <div className="row">
          <h2 className="h6 w-100 rounded p-2 mt-2 mb-3 dashHeading">All lectures available to students</h2>
        </div>
        <div className="row px-3">
          <div className="filterDiv w-100 rounded p-3">
            <p className="text-primary w-100 mb-0 ">Showing {filteredLectures.length} {category || 'lectures'}</p>
            <div className="w-100 d-flex">
              <FormControl className="filterInput d-flex justify-content-center">
                ...
                >
                  ...
                  <MenuItem name="category" value="lectures"> Category: Lectures </MenuItem>
                  <MenuItem name="category" value="courses"> Category: Courses </MenuItem>
                </Select>
              </FormControl>
              <FormControl className="filterInput d-flex justify-content-center">
                <Select
                ...
                >
                  ...
                  <MenuItem name="time" value="nearest"> {category !== 'courses' ? 'Sort: Nearest' : 'Sort: Newest'} </MenuItem>
                  <MenuItem name="time" value="farthest"> {category !== 'courses' ? 'Sort: Farthest' : 'Sort: Oldest'} </MenuItem>
                </Select>
              </FormControl>
              {
                category !== 'courses' ? (
                  <FormControl className="filterInput d-flex justify-content-center">
                    ...
                    >
                      ...
                      <MenuItem name="price" value="free"> Price : Free </MenuItem>
                      <MenuItem name="price" value="0-1000"> Price : Under Rs 1,000 </MenuItem>
                      <MenuItem name="price" value="1000-2000"> Price : Between Rs 1,000 et Rs 2,000 </MenuItem>
                      <MenuItem name="price" value="2000+"> Price : More than Rs 2,000 </MenuItem>
                    </Select>
                  </FormControl>
                ) : null
              }
                ...
                category !== 'courses' ? (
                  <FormControl className="filterInput d-flex justify-content-center">
                    ...
                    >
                      ...
                      <MenuItem name="status" value="live"> Happening now </MenuItem>
                      <MenuItem name="status" value="thisweek"> Happening this week </MenuItem>
                    </Select>
                  </FormControl>
                ) : null
              }
        ...
        <div className="row pb-5">
          {
            !isEmpty(lectures) ? filteredLectures.length ? filteredLectures.map(({
              _id, user: owner, title, subject, classRoom, duration, price, keywords, description, lectureSeries, subscribers, startTime, meetingID, meetingInfo, endTime, name, whatsapp, image,
            }, index) => {
                    ...
                    <div className="card card-cascade horizontal shadow-sm d-flex flex-row">
                      <div className="view overlay horizontal">
                        <img
                          ...
                          alt="Card cap"
                        />
                        <a href="" onClick={(e) => { e.preventDefault(); Router.push(`/dashboard/lectureseries/view/${_id}`); }}>
                        ...
                        <div className="w-100 d-flex flex-column">
                          <div className="d-flex flex-row justify-content-between align-items-center">
                            <div className="d-flex flex-row align-items-center">
                              <h6
                                onClick={(e) => { e.preventDefault(); Router.push(`/dashboard/lectureseries/view/${_id}`); }}
                                style={{cursor: 'pointer'}}
                                className="h6 indigo-text mt-2 text-left text-uppercase font-weight-bold">{name} </h6>
                          ...
                          <div className="d-flex flex-row">
                            ...
                            >
                              ...
                              <span className="strong-grey-text">{`${classRoom ? classRoom.name : 'A Level'}`}</span>
                            </div>
                          </div>
                          <div className="dark-grey-text w-100 px-3 text-justify small-plus">
                            {description ? truncate(180, description) : 'This course is for students that want to learn mathematics every tuesday.'}
                          </div>
                        </div>
                        <div className="d-flex flex-row justify-content-between py-1">
                          <div className="dark-grey-text">
                            <ul className="dark-grey-text list-unstyled list-inline mb-0 d-flex justify-content-start">
                              <li onClick={(e) => { e.preventDefault(); Router.push(`/dashboard/lectureseries/view/${_id}`); }} className="small dark-grey-text list-inline-item pr-2 white-text scheduled">
                        ...
                        <div className="d-flex flex-row justify-content-between align-items-center">
                          <div className="d-flex flex-row align-items-center">
                            <h6
                              onClick={(e) => { e.preventDefault(); Router.push(`/dashboard/lectures/view/${_id}`); }}
                              style={{cursor: 'pointer'}}
                              className="h6 indigo-text mt-2 text-left text-uppercase font-weight-bold">{title} </h6>
                            <span className="d-table lecture-badge badge-new badge badge-pill info-color ml-2">NEW</span>
                            {
                              !(index % 2) ? (
                                <span className="d-table lecture-badge badge-live badge badge-pill danger-color ml-2 mr-2">LIVE</span>
                          ...
                          <div className="subscription d-flex flex-column">
                            {
                              owner._id === user._id && meetingInfo.running[0] === 'false' ? (
                                <button
                      ...
                      <div className="d-flex flex-row justify-content-between py-1">
                        <div className="dark-grey-text">
                          <ul className="dark-grey-text list-unstyled list-inline mb-0 d-flex justify-content-start align-items-center">
                            ...
                            <li className="small dark-grey-text list-inline-item">
                              {
                                whatsapp ? (
                                  <a
                                    ...
                                    <i style={{fontSize: '.7rem'}} className="fa fa-whatsapp mr-1" /> join group
                                  </a>
                                ) : null
                              }
                            </li>
                            <li className="small dark-grey-text list-inline-item">
                              <button
                                onClick={() => { navigator.clipboard.writeText(`https://schoolx.pk/dashboard/lectures/view/${_id}`); handleCopy(`lecture${_id}`); }}
                                rel="noopener noreferrer"
                                type="button"
                                style={{color: '#fff'}}
                                className="btn btn-sm btn-info waves-effect m-0 px-2 py-1">
                                <i style={{fontSize: '.7rem'}} className="fa fa-clone mr-1" /> link
                              </button>
                            </li>
                            <li className="small dark-grey-text list-inline-item">
                              {
                                copied[`lecture${_id}`] ? (
                                  <p style={{color: 'green'}} className="small mb-0">Link copied!</p>
                                ) : null
                              }
                            </li>
                          </ul>
              ...
              <div className="w-100">
                <div style={{height: '100px'}} className="d-flex justify-content-center align-items-center w-100">
          ...
          <div className="card card-cascade wider reverse my-4">
            ...
            <div className="card-body card-body-cascade text-center">
              ...
              <h4 className="card-title">
                <strong>Learning made easy</strong>
              </h4>
              {/* Subtitle */}
              <h6 className="font-weight-bold indigo-text py-2">You will never have to learn alone, join our community!</h6>
              {/* Text */}
              <p className="card-text">
                Lorem ipsum dolor sit amet, consectetur adipisicing elit. Exercitationem
                perspiciatis voluptatum a, quo nobis, non commodi quia repellendus sequi
                nulla voluptatem dicta reprehenderit, placeat laborum ut beatae ullam
                suscipit veniam.
              </p>
              {/* Linkedin */}
