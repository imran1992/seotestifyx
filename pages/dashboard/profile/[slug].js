...
import {
  updateUser, getUser, getClasses, changePassword, getLectures,
} from '@utils/API';
...
const Dashboard = () => {
  ...
  return (
    <main className={owner.role === 'student' ? 'pl-0 pt-5 w-75 mx-auto mainStudent' : 'pt-5 mx-lg-5'}>
      {notifMessage ? (
        <div className={`alert alert-${notifMessageType} mt-4 mb-3 mx-4 w-100`} role="alert">
          {notifMessage}
        </div>
      ) : null}
      {
                    ...
                    <div className="row flex-center px-2 mt-4 ">
                      ...
                      <h6 style={{fontWeight: 'bold'}} className={itsme ? 'w-100 text-right text-primary profileValue' : 'w-100 text-center text-primary profileValue'}>{`${months[date.getMonth()]} ${date.getFullYear()}`}</h6>
                      {
                        role === 'teacher' ? <div className="w-100">
                          ...
                          <h6 onClick={() => Router.push(`/dashboard/profile/lectures/${user._id}`)} style={{fontWeight: 'bold'}} className={itsme ? 'w-100 text-right text-primary profileValue scheduled' : 'w-100 text-center text-primary profileValue scheduled'}>{`${filter(lectures.data, (found) => found.user._id === _id).length} (see list)`}</h6>
                        </div> : null
                      }
                      <h6 style={{borderBottom: '1px solid #ccc'}} className="w-100 text-left text-secondary mb-0"><small>Class: </small></h6>
                      <h6 style={{fontWeight: 'bold'}} className={itsme ? 'w-100 text-right text-primary profileValue' : 'w-100 text-center text-primary profileValue'}>{(classRoom ? classRoom.name : '') || (classes.data.find((found) => found._id === classRoom) || {}).name}</h6>
              ...
              <div className={itsme ? 'col-lg-8 mb-4' : 'd-none'}>
                <div className="card card-cascade narrower">
                  ...
                  <div className="card-body card-body-cascade text-center">
                    ...
                    <FormControl className="lSeriesInput d-flex justify-content-left ml-2">
                      ...
                      >
                        ...
                        {!isEmpty(classes) ? classes.data.map(({_id, name}) => (
                  ...
                  <div className="container">
                    {notifMessage && notifMessageCat === 'info' ? (
                      <div className={`alert alert-${notifMessageType} mb-3 mx-4`} role="alert">
                        {notifMessage}
                      </div>
                    ) : null}
                ...
                <div className="card card-cascade narrower mt-5">
                  ...
                  {notifMessage && notifMessageCat === 'password' ? (
                    <div className={`alert alert-${notifMessageType} mb-3 mx-4`} role="alert">
                      {notifMessage}
                    </div>
                  ) : null}
