...
const Dashboard = () => {
  ...
  const EnrollToCourse = () => {
    if (isEmpty(user))
      Router.push({
        pathname: "/login",
        query: { returnto: `/lectureseries/view/${slug}` },
      });
    else
      enrollToCourse({
        id: dataSource["_id"],
  ...
  return (
    ...
    >
      {!isEmpty(dataSource) && typeof dataSource["name"] != undefined ? (
        <Head>
          <title>
            {dataSource["name"]} | SchoolX, the leading online learning platform
            in Pakistan
          </title>
          <meta
            property="og:url"
          ...
          <meta
            name="description"
            content={`Learn ${dataSource["name"]} by ${
              dataSource["teacher"] != undefined
                ? dataSource["teacher"]["name"]
                : ""
            }. Schoolx is a complete learning platform for your needs ✓ Learn various subjects as a student ✓ Teach your skills and at your own pace as a teacher`}
          />
      ...
      {loading || isLoading ? (
        ...
        >
          ...
          <div
            className={classes.courseStatusChipsContainer}
            typeof="CourseInstance"
          >
            <Chip
              label="ONGOING"
