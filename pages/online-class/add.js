...
const Dashboard = () => {
  ...
  const validLecture =
    name &&
    subjectId &&
    subjects.filter(item => item.classRoomId == input.classRoomId).findIndex(k => k._id == subjectId) != -1 &&
    classRoomId &&
    duration &&
    // price &&
    keywords &&
  ...
  const addLecture = () => {
    ...
    createLecture({ ...input, startTime, teacherId: user._id }).then(response => {
      const { ok, data, problem } = response;
      if (ok) {
        setNotifMessage(
          "Lecture successfully added. You are being redirected to lectures list."
        );
  ...
  useEffect(() => {
    ...
    getLectureSeries().then(response => {
      const { ok, data, problem } = response;
      if (ok) {
        setLectureSeriesData(
          filter(data.data, found => found.teacherId === user._id)
        );
      } else {
        setNotifMessageType("error");
        if (data) setNotifMessage(data.message || problem);
          ...
          <div className="d-md-flex flex-md-fill">
            ...
            <FormControl className="lSeriesInput d-flex justify-content-center ml-2">
              <Select
                ...
                displayEmpty
              >
                <MenuItem name="subjectId" key="void" value="" disabled>
                  Choose subject
                </MenuItem>
