...
const Dashboard = () => {
  ...
  const validLecture =
    (!isEmpty(input) || timeEdited) &&
    (input.courseId || courseId) &&
    subjects
      .filter((item) => item.classRoomId == input.classRoom)
      .findIndex((k) => k._id == subjectId) != -1;
  ...
  useEffect(() => {
    getLecture(slug).then(({ ok, data, problem }) => {
      if (ok) {
        setLecture(data);
  ...
  useEffect(() => {
    if (lecture.classRoomId) {
      // alert(2)
      handleInputChange({
        target: { value: lecture.classRoomId, name: "classRoom" },
        currentTarget: { value: lecture.classRoomId, name: "classRoom" },
      });
    }
  }, [lecture]);
          ...
          <ol className="breadcrumb">
            <button
              ...
              onClick={(e) => {
                e.preventDefault();
                Router.push(
                  pathname.replace("/edit", "").replace("[slug]", slug)
                );
              }}
